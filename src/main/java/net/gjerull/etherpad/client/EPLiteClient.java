 ...
 * <code>
 * EPLiteClient api =
 * new EPLiteClient("http://etherpad.mysite.com", "FJ7jksalksdfj83jsdflkj");
 * <br />
...
public class EPLiteClient {
    ...
    /**
     * Initializes a new net.gjerull.etherpad.client.EPLiteClient
     * object. The default Etherpad Lite API version (in
     * DEFAULT_API_VERSION) will be used.
     *
     * @param url    an absolute url, including protocol, to the
     *               EPL api
    ...
    /**
     * Initializes a new net.gjerull.etherpad.client.EPLiteClient
     * object. The specified Etherpad Lite API version will be
     * used.
     *
     * @param url        an absolute url, including protocol, to
     *                   the EPL api
     * @param apiKey     the API Key
    ...
    // Groups
    // Pads may belong to a group.
    // These pads are not considered "public", and won't
    // be available through the Web UI without a session.

    /**
     * Creates a new Group. The group id is returned in "groupID"
     * in the Map.
     *
    ...
    /**
     * Creates a new Group for groupMapper if one doesn't already
     * exist. Helps you map your application's groups to Etherpad
     * Lite's groups. the group id is returned in "groupID" in
     * the Map.
     *
     * @param groupMapper your group mapper string
     * @return Map with groupID
     */
    public final Map createGroupIfNotExistsFor(
            final String groupMapper) {
        Map<String, Object> args = new HashMap<>();
        args.put("groupMapper", groupMapper);
        return this.connection.post("createGroupIfNotExistsFor",
                args);
    }
    ...
    /**
     * Lists all existing groups. The group ids are returned in
     * "groupIDs".
     *
    ...
    /**
     * Create a new author with the given name. The author id is
     * returned in "authorID".
     *
     * @param name string
     * @return Map with authorID
     */
    ...
    /**
     * Creates a new Author for authorMapper if one doesn't
     * already exist. Helps you map your application's authors to
     * Etherpad Lite's authors. The author id is returned in
     * "authorID".
     *
    ...
    public final Map createAuthorIfNotExistsFor(
        ...
        return this.connection.post("createAuthorIfNotExistsFor",
                args);
    }
     ...
     */
    public final Map createAuthorIfNotExistsFor(
            final String authorMapper, final String name) {
        Map<String, Object> args = new HashMap<>();
        args.put("authorMapper", authorMapper);
        args.put("name", name);
        return this.connection.post("createAuthorIfNotExistsFor",
    ...
    /**
     * List the ids of pads the author has edited. They will be
     * in an array inside "padIDs".
     *
    ...
    public final String getAuthorName(final String authorId) {
        ...
        return (String) this.connection
                .getObject("getAuthorName", args);
    }

    // Sessions
    // Sessions can be created between a group and an author.
    // This allows an author
    // to access more than one group. The sessionID will be set
    // as a
    // cookie to the client and is valid until a certain date.
    // Only users with a
    // valid session for this group, can access group pads. You
    // can create a
    // session after you authenticated the user at your web
    // application, to give
    // them access to the pads. You should save the sessionID of
    // this session
    // and delete it after the user logged out.

    /**
     * Create a new session for the given author in the given
     * group, valid until the given UNIX time. <br />
     * Example:<br />
     * <br />
     * <code>
     ...
     * @param validUntil long UNIX timestamp <strong>in
     *                   seconds</strong>
    ...
    /**
     * Create a new session for the given author in the given
     * group valid for the given number of hours. <br />
     * Example:<br />
     * <br />
     * <code>
     * // in 2 hours<br />
    ...
    public final Map createSession(final String groupID,
        ...
        long inNHours = ((new Date()).getTime()
                + ((long) sessionDuration * SIXTY * SIXTY
                        * THOUSAND))
                / THOUSAND;
    ...
    /**
     * Create a new session for the given author in the given
     * group, valid until the given datetime. <br />
     * Example:<br />
     * <br />
     * <code>
     * import java.util.Date;<br />
    ...
    public final Map createSession(final String groupID,
        ...
        long seconds = validUntil.getTime() / THOUSAND;
     ...
     * sessionIDs keys, with values of Maps containing groupID,
    ...
    /**
     * List all the sessions IDs belonging to an author. Returned
     * as a Map of sessionIDs keys, with values of Maps
     * containing groupID, authorID, and validUntil.
     *
     * @param authorID string
     * @return Map
     */
    public final Map listSessionsOfAuthor(
    ...
    /**
     * Returns a Map containing the latest revision of the pad's
     * text. The text is stored under "text".
     *
    ...
    /**
     * Returns a Map containing the a specific revision of the
     * pad's text. The text is stored under "text".
     *
     * @param padId the pad's id string
     * @param rev   the revision number
     * @return a Map with the text content of pad in given
     *         revision
     */
    public final Map getText(final String padId,
            final long rev) {
     ...
     */
    public final void setText(final String padId,
            final String text) {
        Map<String, Object> args = new HashMap<>();
        args.put("padID", padId);
        args.put("text", text);
    ...
    /**
     * Creates a new revision with the given text appended to the
     * existing text.
     *
     * API >= 1.2.13
     *
     ...
     * @param text  the pad's new text
     */
    public final void appendText(final String padId,
            final String text) {
        Map<String, Object> args = new HashMap<>();
        args.put("padID", padId);
        args.put("text", text);
    ...
    /**
     * Returns a Map containing the current revision of the pad's
     * text as HTML. The html is stored under "html".
     *
     * @param padId the pad's id string
    ...
    /**
     * Returns a Map containing the a specific revision of the
     * pad's text as HTML. The html is stored under "html".
     *
     * @param padId the pad's id string
     * @param rev   the revision number
     * @return a Map with the HTML content of pad in given
     *         revision
     */
    public final Map getHTML(final String padId,
            final long rev) {
     ...
     */
    public final void setHTML(final String padId,
            final String html) {
        Map<String, Object> args = new HashMap<>();
        args.put("padID", padId);
        args.put("html", html);
     ...
     */
    public final String getRevisionChangeset(
            final String padId) {
        Map<String, Object> args = new HashMap<>();
        args.put("padID", padId);
        return (String) this.connection
                .getObject("getRevisionChangeset", args);
    }
    ...
    /**
     * Returns the chat history of pad with index between start
     * and end.
     *
     * API >= 1.2.7
     *
    ...
    /**
     * Returns the chatHead (last number of the last
     * chat-message) of the pad.
     *
     * API >= 1.2.7
     *
     * @param padId the pad's id string
    ...
    /**
     * Creates a chat message, saves it to the database and sends
     * it to all connected clients of this pad, using the current
     * time as timestamp.
     *
     * API >= 1.2.12
     *
    ...
    /**
     * Creates a chat message, saves it to the database and sends
     * it to all connected clients of this pad.
     *
     * API >= 1.2.12
     *
     * @param padId    the pad's id string
    ...
    /**
     * Returns the number of revisions of this pad. The number is
     * in "revisions".
     *
     * @param padId the pad's id string
    ...
    public final Map getSavedRevisionsCount(final String padId) {
        Map<String, Object> args = new HashMap<>();
        args.put("padID", padId);
        return this.connection.get("getSavedRevisionsCount",
                args);
    }
    ...
    /**
     * Returns the list of users that are currently editing this
     * pad. A padUser has the values: "colorId", "name" and
     * "timestamp".
     *
     * @param padId the pad's id string
    ...
    /**
     * Copies a pad with full history and chat. If the
     * destination exists the copy will fail.
     *
     * API >= 1.2.8
     *
     ...
     * @param sourcePadId      the id of the source pad
     * @param destinationPadId the id of the destination pad
     * @param force            if force is true and the
     *                         destination pad exists it will be
     *                         overwritten.
     */
    public final void copyPad(final String sourcePadId,
    ...
    /**
     * Sets the pad's public status. This is only applicable to
     * group pads.
     *
     * @param padId        the pad's id string
    ...
    /**
     * Sets the pad's password. This is only applicable to group
     * pads.
     *
     * @param padId    the pad's id string
    ...
    /**
     * Returns true if the connection is using SSL/TLS, false if
     * not.
     *
     * @return boolean
     */
